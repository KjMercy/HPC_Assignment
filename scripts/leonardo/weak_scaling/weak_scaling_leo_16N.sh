...
start_time=$(date +%s.%N)
srun --ntasks=$nt --cpus-per-task=14 --cpu-bind=cores  ./main -x $x -y $y -p 0 -o 0 -e 300 -v 1 > ../outputs/output_strong_16_node_8taskpernode_14cpupertask.log
end_time=$(date +%s.%N)
tot_time=$(echo "$end_time - $start_time" | bc)
echo "Total_time: ${tot_time}" >> ../outputs/output_strong_16_node_8taskpernode_14cpupertask.log